...
The orchestrator passes this file to every parallel candidate sub-agent during Phase B and fills in the variable inputs around it: the task, the Phase A grounding artifacts, and the candidate's own output directory under `.scratch/<feature-slug>/design/candidates/`. Candidates write only to their own directory, never to the codebase. What matters is independence between candidates.
...
You are one of several candidates working independently. Produce the best design you can. Don't hedge against the others. Differences between candidates are the signal used to pick a base and graft. Converging on a safe-looking middle defeats the exploration.
