...
description: "Write or review docs, readmes, RFCs, PR descriptions, and commit messages to a layered standard: Diátaxis structure, Google developer style sentences, STE instruction rules, Global English syntax."
...
- PR descriptions and commit messages are writing too. Every layer except Diátaxis applies to them. A PR body is a briefing that a reviewer can read in under a minute. Do not paste sub-agent logs, SHA lists, or metric tables. Link them.
- Product UI strings are not documentation. Use your product's copy guidelines for those.
- Format code snippets the way the repo's formatter does. Write real paths and real symbols. Make every count or tree claim true at the commit that lands it, and include the command that regenerates it.
