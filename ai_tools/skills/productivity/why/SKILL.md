---
name: why
description: "Answer 'why does X work this way' or 'why did we pick Y' with cited evidence: queries every available history source (git, issue tracker, docs, chat, observability, error tracking, analytics) in parallel and returns a confidence-weighted read on the decisions and tradeoffs."
...
Reading the code answers what it does and how it works. `why` answers what forces led to its shape, which the code cannot tell you.
...
Before spawning investigators, list the MCP servers and tools available to you in this session.
...
Spawn each investigator as a general-purpose sub-agent with the same tool access you have, so MCP-backed investigators can reach their source. Investigators read only; they write nothing.
...
Spawn one synthesizer sub-agent with the same tool access, because its quality check spot-verifies citations, which can require MCP access. It writes nothing.
